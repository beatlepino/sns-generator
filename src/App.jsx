...
export default function App() {
  ...
  const shortenUrl = async (index) => {
    ...
    try {
      ...
      if (data.shorturl) {
        ...
        setUrls(newUrls);
        setMessage({ type: 'success', text: 'URL短縮成功' });
      }
    } catch (e) {
  ...
  const generate = async () => {
    if (!API_KEY) {
      setMessage({ type: 'error', text: 'APIキー未設定です' });
      return;
    }
    setIsLoading(true);
    const appended = `${urls.filter(u=>u).join('\n')}\n${hashtags}\n${mention}`.trim();
    const prompt = `あなたはSNSプロです。イベント名:${eventName}, 詳細:${eventDetails}, 設定:${CHARACTER_PROMPTS[charKey]}, 付随:${appended}。180文字以内で告知文を作成してください。`;

    try {
      const res = await fetch('[https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent](https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent)', {
        method: 'POST',
